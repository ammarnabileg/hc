...
@section('content')
    ...
    <div class="panel w-full max-w-lg">
        <div class="cluster mb-4">
            ...
            <h1>{{ setting('onboarding.referral.title', 'هل دعاك شخص ما؟') }}</h1>
        </div>
        ...
        <form method="post" action="{{ route('onboarding.referral.apply') }}" class="stack" style="gap: 14px">
            @csrf
            <label class="block">
        ...
        <form method="post" action="{{ route('onboarding.referral.skip') }}" class="mt-3 text-center">
            @csrf
            <button class="btn text small underline" style="color: var(--muted)">
