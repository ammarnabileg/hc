...
@section('title', 'اجتماعات التطوّع')
...
@section('content')
    <x-page-header title="اجتماعات التطوّع"
                   subtitle="نظرة عرضيّة على الأقسام كلّها — الاجتماعات تُدار من لوحة التطوّع، وهنا تُراجَع وتُنهى وتُصدَّر."
                   :breadcrumbs="[['label' => 'لوحة الإدارة', 'url' => route('admin.dashboard')], ['label' => 'اجتماعات التطوّع']]">
        <x-slot:action>
            @if ($canExport)
                <a href="{{ route('admin.meetings.export', request()->query()) }}"
                   class="btn rounded-xl px-4 py-2 text-sm font-semibold motion-standard"
                   style="background: var(--color-brand-500); color: #04201c">تصدير الحضور</a>
            @endif
        </x-slot:action>
    </x-page-header>
            ...
            <table class="w-full text-sm"
                ...
                <thead>
                    <tr style="background: var(--surface-sunken)">
                        ...
                        <th class="text-start px-4 py-3 font-semibold">النطاق</th>
                        <th class="text-start px-4 py-3 font-semibold">الموعد</th>
                        <th class="text-start px-4 py-3 font-semibold">الحضور</th>
                        <th class="text-start px-4 py-3 font-semibold">نافذة التسجيل</th>
                ...
                <tbody>
                    @foreach ($meetings as $meeting)
                        <tr style="border-top: 1px solid var(--border)">
                            ...
                            <td class="px-4 py-3 text-xs">{{ $meeting->entity?->name_ar ?? 'كلّ المتطوّعين' }}</td>
                            ...
                            <td class="px-4 py-3">
                                <x-state-badge :state="match ($meeting->status) { 'ended' => blank($meeting->minutes) ? 'warn' : 'ok', 'running' => 'warn', default => 'idle' }"
                                               :label="blank($meeting->minutes) && $meeting->status === 'ended' ? 'بانتظار المحضر' : ($statuses[$meeting->status] ?? $meeting->status)" />
                            </td>
                            <td class="px-4 py-3">
            ...
            @foreach ($meetings as $meeting)
                <article class="card p-4">
                    ...
                    <dl class="mt-2 text-xs space-y-1" style="color: var(--text-muted)">
                        <div><dt class="inline">النطاق:</dt> <dd class="inline">{{ $meeting->entity?->name_ar ?? 'كلّ المتطوّعين' }}</dd></div>
                        <div><dt class="inline">الموعد:</dt> <dd class="inline">{{ $meeting->scheduled_at?->format('Y-m-d H:i') }}</dd></div>
                        <div><dt class="inline">الحضور:</dt> <dd class="inline">{{ $counts[$meeting->id]['registered'] ?? 0 }} / {{ $counts[$meeting->id]['invited'] ?? 0 }}</dd></div>
                    </dl>
    ...
    @if ($canSettings)
        @include('admin.screens24.settings', [
            ...
            'blockTitle' => 'إعدادات مرآة الاجتماعات',
        ])
    @endif
@endsection
...
@push('modals')
    @if ($canEnd)
        <x-modal id="end-modal" title="إنهاء الاجتماع وفتح نافذة الحضور">
            <form method="post" action="{{ route('admin.meetings.index') }}" data-end-form>
                @csrf
                <p class="text-sm mb-3" style="color: var(--text-muted)">
                    الإنهاء بيفتح نافذة تسجيل الحضور — والتسجيل خلال أوّل ساعات بيدّي درجة التزام أعلى.
                </p>

                <label class="block text-sm font-semibold mb-1" for="end-hours">عدد ساعات النافذة</label>
                ...
                <label class="block text-sm font-semibold mb-1" for="end-minutes">المحضر (اختياريّ الآن، مطلوب للتوثيق)</label>
                ...
                <button type="submit" class="btn rounded-xl px-4 py-2 text-sm font-semibold motion-standard"
                        style="background: var(--color-brand-500); color: #04201c">أنهِ الاجتماع</button>
            </form>
        </x-modal>
    @endif

    @if ($canGrant)
        <x-modal id="grant-modal" title="منح حضور استثنائيّ">
            <form method="post" action="{{ route('admin.meetings.index') }}" data-grant-form>
                @csrf
                <label class="block text-sm font-semibold mb-1" for="grant-user">كود العضو أو رقمه</label>
                <input type="number" name="user_id" id="grant-user" required min="1"
                       class="w-full rounded-xl px-3 py-2 text-sm mb-3"
                       style="background: var(--surface-sunken); border: 1px solid var(--border); color: var(--text)">

                <label class="block text-sm font-semibold mb-1" for="grant-reason">سبب المنح</label>
                <textarea name="reason" id="grant-reason" rows="2" required maxlength="500"
                          placeholder="مثال: انقطاع النت أثناء الاجتماع — أثبت حضوره بالتسجيل."
                          class="w-full rounded-xl px-3 py-2 text-sm mb-4"
                          style="background: var(--surface-sunken); border: 1px solid var(--border); color: var(--text); resize: vertical"></textarea>

                <button type="submit" class="btn rounded-xl px-4 py-2 text-sm font-semibold motion-standard"
                        style="background: var(--color-brand-500); color: #04201c">امنح الحضور</button>
            </form>
        </x-modal>
    @endif
@endpush
