...
<section class="card p-4 md:p-5">
    <div class="flex items-center justify-between gap-3 flex-wrap mb-3">
        <h2 class="font-bold">{{ setting('admin.gamification.tabs.levels.almstwyat_watbat_xp', 'المستويات وعتبات XP') }}</h2>
        @can('achievements.edit')
            <button type="button" data-modal-open="level-modal"
                    ...
                    style="background: var(--color-brand-500); color: #04201c">{{ setting('admin.gamification.tabs.levels.mstwa_2', '+ مستوى') }}</button>
        @endcan
    </div>
            ...
            <span class="flex items-center gap-2 shrink-0">
                @can('achievements.edit')
                    <button type="button" class="text-xs underline" data-level-edit
                            ...
                            data-name="{{ $level->name_ar }}" data-xp="{{ $level->min_xp }}">{{ setting('admin.gamification.tabs.levels.tadyl', 'تعديل') }}</button>
                @endcan
...
@push('modals')
    @can('achievements.edit')
        <x-modal id="level-modal" :title="setting('admin.gamification.tabs.levels.mstwa', 'مستوى')">
            <form method="post" action="{{ route('admin.gamification.levels.save') }}">
                @csrf
                <input type="hidden" name="id" id="level-id">

                <label class="block text-sm font-semibold mb-1" for="level-num">{{ setting('admin.gamification.tabs.levels.rqm_almstwa', 'رقم المستوى') }}</label>
                <input type="number" min="1" name="level" id="level-num" required
                       class="w-full rounded-xl px-3 py-2 text-sm mb-3"
                       style="background: var(--surface-sunken); border: 1px solid var(--border); color: var(--text)">

                <label class="block text-sm font-semibold mb-1" for="level-name">{{ setting('admin.gamification.tabs.levels.alasm', 'الاسم') }}</label>
                <input type="text" name="name_ar" id="level-name" required maxlength="80"
                       class="w-full rounded-xl px-3 py-2 text-sm mb-3"
                       style="background: var(--surface-sunken); border: 1px solid var(--border); color: var(--text)">

                <label class="block text-sm font-semibold mb-1" for="level-xp">{{ setting('admin.gamification.tabs.levels.atba_xp', 'عتبة XP') }}</label>
                ...
                <button type="submit" class="btn mt-4 rounded-xl px-4 py-2 text-sm font-semibold"
                        style="background: var(--color-brand-500); color: #04201c">{{ setting('admin.gamification.tabs.levels.ahfz', 'احفظ') }}</button>
            </form>
        </x-modal>
    @endcan
@endpush
