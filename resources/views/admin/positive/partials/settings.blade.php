...
<details class="card p-4 md:p-5 mt-4">
            ...
            @foreach ($settings as $row)
                <label class="block {{ $row['type'] === 'lines' ? 'md:col-span-2' : '' }}">
                    ...
                    @elseif ($row['type'] === 'bool')
                        <span class="flex items-center gap-2">
                            ...
                            <input type="checkbox" name="settings[{{ $row['key'] }}]" value="1"
                                   @checked($row['value'] === '1')>
                            <span class="text-sm">{{ setting('admin.positive.partials.settings.mfal', 'مفعّل') }}</span>
                        </span>
                    ...
                    <span class="block text-xs mt-1" style="color: var(--text-muted)">
                        {{ $row['hint'] ?: setting('admin.positive.partials.settings.alaftrady', 'الافتراضيّ: ').$row['default'] }}
                        @if ($row['modified'])
                            · <span style="color: var(--color-state-warn)">{{ setting('admin.positive.partials.settings.madl', 'معدَّل') }}</span>
                        @endif
                    </span>
                </label>
            @endforeach
    ...
    <form method="post" action="{{ route('admin.positive.settings.reset') }}" class="mt-3">
        @csrf
        <button type="submit" class="text-xs underline" style="color: var(--text-muted)"><x-icon name="refresh" size="16" /> {{ setting('admin.positive.partials.settings.rja_aliadadat_klha_llaftrady', 'رجّع الإعدادات كلّها للافتراضيّ') }}</button>
    </form>
</details>
