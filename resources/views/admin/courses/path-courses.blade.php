...
@section('title', 'تدريبات المسار')

@section('content')
    {{-- إدارة تدريبات المسار: بحث/سحب-ترتيب/إضافة/حذف (12.4-أ) --}}
    <x-page-header
        :title="'تدريبات: '.$path->name_ar"
        subtitle="رتّب التدريبات جوّه المسار — والإزالة هنا بتفكّ الارتباط بس، التدريب بيفضل موجود."
        :breadcrumbs="[
            ['label' => 'المسارات', 'url' => route('admin.paths.index')],
            ['label' => $path->name_ar],
        ]">
        <x-slot:action>
            @can('paths.edit')
                <button type="button" data-modal-open="attach-courses"
                        ...
                        style="background: var(--color-brand-500); color: #04201c">+ أضِف تدريبات</button>
            @endcan
        </x-slot:action>
    </x-page-header>

    <form method="get" class="mb-4">
        <input type="search" name="q" value="{{ $search }}" placeholder="ابحث في تدريبات المسار…"
               class="w-full md:w-80 rounded-xl px-3 py-2 text-sm"
               style="background: var(--surface-sunken); border: 1px solid var(--border); color: var(--text)">
    </form>

    @if ($courses->isEmpty())
        <x-empty message="المسار لسّه فاضي — ضيف أوّل تدريب." />
    @else
                        ...
                        @can('paths.edit')
                            <form method="post" action="{{ route('admin.paths.courses.detach', [$path, $course]) }}"
                                ...
                                <button class="btn rounded-xl px-3 py-2 text-sm"
                                        style="background: var(--surface-sunken); color: var(--color-state-danger)">إزالة</button>
                            </form>
                        @endcan
                    </div>
                </div>
    ...
    @can('paths.edit')
        <x-modal id="attach-courses" title="أضِف تدريبات للمسار">
            <p class="text-sm mb-3" style="color: var(--text-muted)">
                التدريب يقدر يكون في أكتر من مسار — إضافته هنا ما بتشيلوش من مساراته التانية.
            </p>

            @if ($attachable->isEmpty())
                <p class="text-sm">مفيش تدريبات جديدة تضيفها دلوقتي.</p>
            @else
                <form method="post" action="{{ route('admin.paths.courses.attach', $path) }}" class="space-y-2">
                    ...
                    <button class="btn w-full rounded-xl px-4 py-3 text-sm font-semibold"
                            style="background: var(--color-brand-500); color: #04201c">أضِف المختار</button>
                </form>
            @endif
        </x-modal>
    @endcan
