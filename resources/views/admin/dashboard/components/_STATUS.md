# 🗂️ `resources/views/admin/dashboard/components` — حالة المجلّد
...
## 🎯 الغرض/المطلوب
<!-- تلقائيّ:بداية:الغرض -->
- **الوظيفة:** مكوّنات بليد خاصّة بالمجلّد الأب لمجلّد `resources/views/admin/dashboard`.
- **البنود الحاكمة (من تعليقات الكود نفسه):** 2.15-د · 2.15-ج · 2.15-أ-7 · 2.16-ج · 2.16 · 2.17-أ · 12.3 · 12.3-2/18 · 12.3-4 · 12.3-7 · 12.3-13 · 12.3-15
...
- **شاشات/أجزاء بليد (4):**
  - `chart-funnel.blade.php`
  - `chart-lines.blade.php` — الرسم داخل حاوية متمرّرة أفقيًّا فلا تمرير أفقيّ للصفحة (2.15-ج)
  - `heat-rows.blade.php`
  - `kpi.blade.php` — الأيقونة SVG من قاموس المنصّة — ممنوع أيّ مكتبة أيقونات (2.16-ج)
...
- **دفعة النصّ المحروق (2026-08-04):** كلّ نصٍّ عربيّ ظاهر في قوالب هذا المجلّد صار `setting('مفتاح', 'النصّ الحاليّ')` — **14 موضعًا محروقًا ⟵ صفر**، و**12 مفتاحًا** معرَّفةً في `database/seeders/AdminScreenText*DemoSeeder.php` (يستدعيها `SettingDefinitionsSeeder` في مسار الإنتاج). ولا حرفَ تغيّر: الافتراضيّ هو النصّ الذي كان. والعتبة أُحكِمت على هذا المجلّد في `docs/hardcoded-text-baseline.json`.
<!-- بيدك:نهاية:الجاري -->
