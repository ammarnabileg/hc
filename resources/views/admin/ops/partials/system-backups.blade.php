@can('backups.create')
    <form method="post" action="{{ route('admin.ops.system.backups.store') }}" class="card p-3 mb-4 flex flex-wrap items-end gap-3">
        @csrf
        <label class="block">
            <span class="block text-sm mb-1">{{ setting('admin.ops.partials.system_backups.nwa_alnskha', 'نوع النسخة') }}</span>
            ...
            <tbody>
                @foreach ($backups as $row)
                    <tr style="border-top: 1px solid var(--border)">
                        ...
                        <td class="p-3">{{ $kinds[$row->kind] ?? $row->kind }}{{ $row->is_scheduled ? setting('admin.ops.partials.system_backups.mjdwla', ' · مجدولة') : '' }}</td>
                        <td class="p-3">{{ app(\App\Services\Admin\Ops\BackupManager::class)->humanSize((int) $row->size_bytes) }}</td>
                        <td class="p-3">{{ $row->author_name ?? setting('admin.ops.partials.system_backups.alnzam', 'النظام') }}</td>
                        <td class="p-3">
                            <x-state-badge :state="$row->status === 'done' ? 'ok' : 'danger'"
                                           :label="$row->status === 'done' ? setting('admin.ops.partials.system_backups.slyma', 'سليمة') : setting('admin.ops.partials.system_backups.fashla', 'فاشلة')" />
                        </td>
                        <td class="p-3">
            ...
            @foreach ($backups as $row)
                <div class="p-3 text-sm" style="border-top: 1px solid var(--border)">
                    ...
                    <div class="text-xs mt-1" style="color: var(--text-muted)">
                        ...
                        {{ \Illuminate\Support\Carbon::parse($row->created_at)->diffForHumans() }} ·
                        {{ $row->author_name ?? setting('admin.ops.partials.system_backups.alnzam', 'النظام') }}
                    </div>
                    <div class="mt-2">
                        @include('admin.ops.partials.system-backup-actions', ['row' => $row])
                    </div>
