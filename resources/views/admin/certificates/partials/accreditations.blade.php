        ...
        <div class="card p-4 flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="text-xs mt-1" style="color: var(--text-muted)">
                    {{ $typeCounts[$accreditation->id] ?? 0 }} نوع شهادة ·
                    {{ $issuedCounts[$accreditation->id] ?? 0 }} شهادة صادرة
                </div>
                @if ($accreditation->verify_note_ar)
                    <div class="text-xs mt-1">في صفحة التحقّق: «{{ $accreditation->verify_note_ar }}»</div>
                @endif
            </div>

            <div class="flex items-center gap-2">
                <x-state-badge :state="$accreditation->is_active ? 'ok' : 'idle'"
                               :label="$accreditation->is_active ? 'نشط' : 'معطّل'" />
                ...
                @can('accreditations.delete')
                    @unless ($accreditation->is_platform)
                        <form method="post" action="{{ route('admin.certificates.accreditations.destroy', $accreditation) }}"
                              onsubmit="return confirm('نشيل الاعتماد ده؟')">
                            @csrf @method('delete')
...
@can('accreditations.create')
    <x-modal id="accreditation-form" title="اعتماد جديد">
        <form method="post" action="{{ route('admin.certificates.accreditations.store') }}" class="space-y-3">
            ...
            <x-form.input name="logo_path" label="الشعار (مسار من مكتبة الوسائط)" />
            <x-form.input name="verify_note_ar" label="نصّ صفحة التحقّق (عربيّ)"
