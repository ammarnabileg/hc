...
<div class="card p-4">
    <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
            <div class="text-sm font-extrabold">بيانات الدول</div>
            <p class="text-xs mt-1" style="color: var(--text-muted)">
                حدّث المصدر ({{ setting('countries.source', 'dr5hn') }}) بعد ما تشوف الفروق بنفسك — وما حدش بيفقد ارتباطه بدولته ولا بمحافظته.
            </p>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-2">
            @can('countries_data.export')
                <a href="{{ route('admin.countries.export') }}" class="rounded-xl px-3 py-2 text-sm"
                   style="background: var(--surface-raised)">تصدير النسخة الحاليّة</a>
            @endcan
            ...
            @can('countries_data.import')
                ...
                <form method="post" action="{{ route('admin.countries.import') }}" enctype="multipart/form-data"
                    ...
                    <input type="file" name="file" accept="application/json" required class="text-xs w-40"
                           aria-label="ملفّ نسخة المصدر (JSON)">
                    <button class="btn rounded-xl px-4 py-2 text-sm font-semibold"
                            style="background: var(--color-brand-500); color: #04201c">استيراد نسخة</button>
                </form>
            @endcan
        </div>
    </div>
    ...
    <ul class="mt-3 text-xs space-y-1" style="color: var(--text-muted)">
        <li>● لا حذف إطلاقًا من هنا — المحذوف من المصدر <strong>يُخفى</strong> بس، وصفّه بيفضل بأرقامه.</li>
...
@if ($report)
    ...
    <div class="card p-4 mt-3">
        <div class="text-sm font-semibold flex items-center gap-2">
            <x-state-badge :state="$report['dry_run'] ? 'idle' : 'ok'" :label="$report['dry_run'] ? 'معاينة' : 'اتنفّذ'" />
            تقرير الدمج
        </div>
        <div class="text-xs mt-2" style="color: var(--text-muted)">
            مضاف {{ $report['added'] }} · معدَّل {{ $report['updated'] }} · مخفيّ {{ $report['hidden'] }} · متروك {{ $report['skipped'] }}
        </div>
        @if ($report['protected'])
            <div class="mt-2 text-xs">
                <span class="font-semibold">محميّة من الحذف ({{ count($report['protected']) }}):</span>
...
@if ($diff && ($diff['added'] || $diff['removed'] || $diff['changed']))
    ...
    <form method="post" action="{{ route('admin.countries.merge') }}" class="card p-4 mt-3">
        ...
        <div class="flex flex-wrap items-center justify-between gap-2">
            <div class="text-sm font-semibold">فروق النسخة {{ $snapshot->version ? '('.$snapshot->version.')' : '' }}</div>
            <x-state-badge state="ok" label="سيُدمج بلا فقد" />
        </div>
                    ...
                    @forelse ($diff[$bucket] as $row)
                        <label class="flex items-start gap-2 rounded-xl p-2 mb-2 text-xs"
                            ...
                            <span class="min-w-0">
                                ...
                                @if ($row['protected'])
                                    <span class="inline-block mt-1"><x-state-badge state="honor" label="محميّة" /></span>
                                @endif
                                @if ($row['users'] > 0)
                                    <span class="block mt-1">مرتبطة بـ{{ $row['users'] }} مستخدم</span>
                                @endif
                            </span>
                        </label>
                    @empty
                        <p class="text-xs" style="color: var(--text-muted)">مافيش.</p>
                    @endforelse
                </section>
            @endforeach
        </div>
        ...
        @can('countries_data.import')
            <div class="flex flex-wrap items-center gap-2 mt-3">
                <button type="submit" name="dry_run" value="1" class="rounded-xl px-4 py-2 text-sm"
                        style="background: var(--surface-raised)">Dry-run (بلا كتابة)</button>
                <button type="submit" class="btn rounded-xl px-4 py-2 text-sm font-semibold"
                        style="background: var(--color-brand-500); color: #04201c">ادمج المختار</button>
            </div>
        @endcan
    </form>
@elseif ($snapshot)
    <div class="card p-4 mt-3">
        <p class="text-sm">النسخة المرفوعة مطابقة لبياناتنا — مافيش فروق. ✓</p>
    </div>
@endif
...
<form method="get" action="{{ route('admin.settings.index') }}" class="card p-3 mt-3 flex flex-wrap items-end gap-3">
    ...
    <label class="block flex-1 min-w-[12rem]">
        <span class="block text-xs mb-1">بحث</span>
        <input type="search" name="cq" value="{{ $filters['q'] }}" placeholder="دولة أو كود ISO…"
               class="w-full rounded-xl px-3 py-2 text-sm"
               style="background: var(--surface-sunken); border: 1px solid var(--border); color: var(--text)">
    </label>

    <label class="block">
        <span class="block text-xs mb-1">الحالة</span>
        <select name="cstatus" class="rounded-xl px-3 py-2 text-sm"
            ...
            <option value="hidden" @selected($filters['status'] === 'hidden')>مخفيّة</option>
        </select>
    </label>

    <label class="flex items-center gap-2 text-sm">
        <input type="checkbox" name="cusers" value="1" @checked($filters['with_users'])> فيها مستخدمون
    </label>

    <button class="rounded-xl px-4 py-2 text-sm" style="background: var(--surface-raised)">تصفية</button>
</form>

@if ($rows->isEmpty())
    <x-empty message="مافيش دول مطابقة — وسّع البحث." />
@else
                ...
                <div class="flex flex-wrap items-center justify-between gap-2">
                    <div class="min-w-0">
                        ...
                        <div class="text-xs mt-0.5" style="color: var(--text-muted)">
                            {{ $country->phone_code ?: '—' }} · {{ $row['governorates'] }} محافظة · {{ $row['users'] }} مستخدم · {{ $country->timezone }}
                        </div>
                    </div>

                    <div class="flex items-center gap-2">
                        <x-state-badge :state="$country->is_active ? 'ok' : 'idle'"
                                       :label="$country->is_active ? 'ظاهرة' : 'مخفيّة'" />
                        @if ($country->sync_hidden_at)
                            <span class="text-xs" style="color: var(--text-muted)">أخفاها الدمج</span>
                        @endif
                    </div>
                </div>
                ...
                @can('countries_data.edit')
                    <form method="post" action="{{ route('admin.countries.update', $country) }}"
                        ...
                        <label class="flex-1 min-w-[10rem]">
                            <span class="block text-xs mb-1">الاسم العربيّ</span>
                            <input type="text" name="name_ar" value="{{ $country->name_ar }}" required
                                   class="w-full rounded-xl px-3 py-2 text-sm"
                                   style="background: var(--surface-sunken); border: 1px solid var(--border); color: var(--text)">
                        </label>
                        <label class="flex items-center gap-2 text-sm">
                            <input type="checkbox" name="is_active" value="1" @checked($country->is_active)> ظاهرة
                        </label>
                        <button class="rounded-xl px-3 py-2 text-sm" style="background: var(--surface-raised)">حفظ</button>
                    </form>
                @endcan
            </div>
        @endforeach
