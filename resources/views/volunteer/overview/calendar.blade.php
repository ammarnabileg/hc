...
@section('content')
    ...
    <x-page-header title="تقويم نشاطي"
                   subtitle="مهامّك واجتماعاتك ومواعيدك في مكان واحد."
                   :breadcrumbs="[
        ...
        <x-slot:action>
            <div class="flex items-center gap-2 text-sm">
                <a class="rounded-xl px-3 py-2" style="background: var(--surface-raised)"
                   ...
                   aria-label="الشهر السابق">‹</a>
                <a class="rounded-xl px-3 py-2" style="background: var(--surface-raised)"
                   href="{{ route('volunteer.calendar') }}">اليوم</a>
                <a class="rounded-xl px-3 py-2" style="background: var(--surface-raised)"
                   href="{{ route('volunteer.calendar', ['m' => $month->copy()->addMonth()->format('Y-m')]) }}"
                   aria-label="الشهر التالي">›</a>
            </div>
        </x-slot:action>
    </x-page-header>

    <div class="text-sm mb-3" style="color: var(--text-muted)">
        {{ $month->translatedFormat('F Y') }} · نافذة النشاط {{ $window->label() }}
    </div>
    ...
    <div class="grid lg:grid-cols-[1fr_18rem] gap-4">
        ...
        <div class="card p-3">
            <div class="text-sm mb-2">اليوم — {{ now()->translatedFormat('l j F') }}</div>

            @if ($today->isEmpty())
                <p class="text-sm" style="color: var(--text-muted)">مفيش مواعيد في المدى ده.</p>
            @else
            ...
            <div class="text-xs mb-1" style="color: var(--text-muted)">ساعات اليوم</div>
            <div class="grid grid-cols-12 gap-0.5">
                @foreach ($hours as $hour)
                    @php $inside = $window->containsHour($hour); @endphp
                    <div class="h-4 rounded-sm cursor-help"
                         title="{{ $hour }}:00 — {{ $inside ? 'داخل نافذة النشاط' : $window->outsideHint() }}"
