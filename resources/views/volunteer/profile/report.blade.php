    ...
    <x-page-header
        ...
        <x-slot:action>
            <button type="button" onclick="window.print()"
                    ...
                    style="background: var(--color-brand-500); color: #04201c">اطبع / احفظ PDF</button>
        </x-slot:action>
    </x-page-header>
    ...
    <section class="card p-5 print-sheet">
        <div class="flex flex-wrap items-start justify-between gap-3 mb-4 pb-4" style="border-bottom: 1px solid var(--border)">
            ...
            <div class="text-sm text-end">
                <div>رقم المرجع: <span class="font-mono">{{ $reference }}</span></div>
                <div style="color: var(--text-muted)">تاريخ الإصدار: {{ $issued_at->format('Y-m-d H:i') }}</div>
                <div style="color: var(--text-muted)">أصدره: {{ $issued_by->shortName() }}</div>
            </div>
        </div>

        <dl class="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            <div>
                <dt style="color: var(--text-muted)">معدّل درجة الالتزام (آخر {{ $days }} يوم)</dt>
                <dd class="font-bold text-lg">{{ $rep_average }}</dd>
            </div>
            <div>
                <dt style="color: var(--text-muted)">درجة الالتزام الحاليّة</dt>
                <dd class="font-bold text-lg">{{ $rep_now }}</dd>
            </div>
            <div>
                <dt style="color: var(--text-muted)">نسبة حضور الاجتماعات</dt>
                <dd class="font-bold text-lg">{{ $attendance['percent'] !== null ? $attendance['percent'].'%' : '—' }}</dd>
            </div>
            <div>
                <dt style="color: var(--text-muted)">المهامّ</dt>
                <dd class="font-bold text-lg">{{ $tasks['approved'] }} / {{ $tasks['total'] }}</dd>
            </div>
            <div>
                <dt style="color: var(--text-muted)">المهامّ المتأخّرة</dt>
                <dd class="font-bold text-lg">{{ $tasks['late'] }}</dd>
            </div>
            <div>
                <dt style="color: var(--text-muted)">متوسّط التقييم</dt>
        ...
        <div class="mt-5 pt-4 text-sm" style="border-top: 1px solid var(--border)">
            <span style="color: var(--text-muted)">الأبلاين الموصي:</span>
            <span class="font-bold">{{ $recommender?->name ?? 'مفيش أبلاين مباشر' }}</span>
        ...
        <p class="mt-4 text-xs" style="color: var(--text-muted)">
            التقرير مسجَّل في سجلّ التدقيق برقم {{ $audit_id }} — أرقامه من مصادرها الواحدة بلا حساب موازٍ.
        </p>
    </section>
@endsection
