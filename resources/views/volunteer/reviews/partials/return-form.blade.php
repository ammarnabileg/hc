...
<form method="post" action="{{ $action }}" class="space-y-3">
    ...
    <label class="block">
        <span class="block text-sm mb-1">الفيدباك المكتوب <span style="color: var(--color-state-danger)">*</span></span>
        <textarea name="review_feedback" rows="4" required
                  placeholder="اكتب بالضبط إيه اللي ناقص وإزاي يتظبط"
    ...
    <label class="block">
        <span class="block text-sm mb-1">تصنيف السبب <span style="color: var(--color-state-danger)">*</span></span>
    ...
    <x-form.input name="fix_hours" label="مهلة الإصلاح (ساعات)" type="number" :value="$fixHours"
                  hint="مهلة مستقلّة بسلّمها المستقلّ — والديدلاين الأصليّ لا يُحتسَب مرّتين." />

    <button type="submit" class="btn w-full rounded-xl px-4 py-2 text-sm font-semibold"
            style="background: var(--color-state-warn); color: #04201c">إرجاع</button>
</form>
