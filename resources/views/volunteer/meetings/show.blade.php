...
@section('content')
    <x-page-header
        :title="$meeting->title"
        :subtitle="$meeting->scheduled_at?->format('Y-m-d · H:i').' — صاحبه '.$meeting->owner?->name"
        :breadcrumbs="[
            ['label' => 'الاجتماعات', 'url' => route('volunteer.meetings')],
            ['label' => $meeting->title],
        ]">
        <x-slot:action>
            ...
            @if ($windowOpen && ($myAttendance->status ?? null) !== 'registered')
                <button type="button" data-modal-open="register-{{ $meeting->id }}"
                        ...
                        style="background: var(--color-brand-500); color: #04201c">تسجيل حضور</button>
            @endif
                ...
                <div class="card absolute end-0 mt-2 w-56 p-2 z-40 space-y-1">
                    @if ($meeting->status !== 'ended' && ! in_array($myAttendance->status ?? null, ['excused', 'excused_settled'], true))
                        <button type="button" data-modal-open="excuse-{{ $meeting->id }}"
                                class="w-full text-start rounded-xl px-3 py-2 text-sm">اعتذار مسبق</button>
                    @endif
                    @if ($canManage && $meeting->status !== 'ended')
                        <button type="button" data-modal-open="end-{{ $meeting->id }}"
                                class="w-full text-start rounded-xl px-3 py-2 text-sm">إنهاء الاجتماع</button>
                    @endif
                    @if ($canManage)
                        <button type="button" data-modal-open="questions-{{ $meeting->id }}"
                                class="w-full text-start rounded-xl px-3 py-2 text-sm">إضافة أسئلة / OTP</button>
                    @endif
                </div>
            </details>
        </x-slot:action>
    ...
    <div class="flex flex-wrap items-center gap-2 mb-4">
        ...
        @if ($windowOpen)
            <x-state-badge :state="$attendance->tierState($meeting)" label="نافذة التسجيل مفتوحة" />
            <span class="text-xs" style="color: var(--text-muted)"
                  data-countdown="{{ $meeting->attendance_closes_at->toIso8601String() }}"
                  data-prefix="باقي">{{ $meeting->attendance_closes_at->diffForHumans() }}</span>
        @endif
    </div>

    <x-tabs :current="$tab" :tabs="[
        ['key' => 'details', 'label' => 'التفاصيل', 'url' => $tabUrl('details')],
        ['key' => 'attendance', 'label' => 'الحضور', 'url' => $tabUrl('attendance')],
        ['key' => 'minutes', 'label' => 'المحضر', 'url' => $tabUrl('minutes')],
        ['key' => 'discussion', 'label' => 'النقاش', 'url' => $tabUrl('discussion')],
    ]" />

    @if ($tab === 'details')
        <div class="card p-4 space-y-3">
            <p class="text-sm whitespace-pre-line">{{ $meeting->description ?: 'مفيش وصف مكتوب للاجتماع ده.' }}</p>
            ...
            <div class="text-xs grid gap-2 sm:grid-cols-2" style="color: var(--text-muted)">
                <span>الكيان: {{ $meeting->entity?->name_ar ?? 'كلّ الكيانات' }}</span>
                <span>الجمهور: {{ ['all' => 'الكلّ', 'sub_entity' => 'قسم فرعيّ'][$meeting->audience] ?? 'قسم' }}</span>
                <span>عدد أسئلة الحضور: {{ $meeting->questions->count() }}</span>
                <span>كود حضور: {{ $meeting->attendance_code ? 'مفعَّل' : 'بلا كود' }}</span>
            </div>
    ...
    @if ($tab === 'attendance')
        @if (! $canSeeFull)
            <p class="text-xs mb-3" style="color: var(--text-muted)">القائمة الكاملة للمخوَّل — وإنت شايف حالتك إنت.</p>
        @endif
            ...
            <div class="card min-w-0 overflow-x-auto hidden md:block">
                <table class="w-full text-sm">
                    <thead style="color: var(--text-muted)">
                        <tr class="text-start">
                            <th class="p-3 text-start">العضو</th>
                            <th class="p-3 text-start">الحالة</th>
                            <th class="p-3 text-start">وقت التسجيل</th>
                            ...
                            <tr style="border-top: 1px solid var(--border)">
                                ...
                                <td class="p-3">
                                    <x-state-badge :state="$row->status === 'registered' ? 'ok' : ($row->status === 'absent' ? 'danger' : 'idle')"
                                                   :label="['registered' => 'سجّل', 'absent' => 'غياب', 'excused' => 'اعتذر', 'excused_settled' => 'اعتذر'][$row->status] ?? $row->status" />
                                </td>
                ...
                @foreach ($rows as $row)
                    <div class="card p-3 text-sm">
                        ...
                        <div class="mt-1 text-xs" style="color: var(--text-muted)">
                            ...
                            القيمة {{ $attendance->valueLabel((float) $row->rep_value) }}
                        </div>
                    </div>
                @endforeach
            </div>
    ...
    @if ($tab === 'minutes')
        <div class="card p-4">
            ...
            @else
                <p class="text-sm" style="color: var(--text-muted)">المحضر لسّه ما اترفعش.</p>
            @endif
    ...
    @if ($tab === 'discussion')
        <form method="post" action="{{ route('volunteer.meetings.posts', $meeting) }}"
            ...
            @csrf
            <textarea name="body" rows="2" required maxlength="4000" placeholder="اكتب بوست للنقاش…"
        ...
        <div class="flex items-center gap-2 mb-3 text-xs">
            <a href="{{ route('volunteer.meetings.show', ['meeting' => $meeting, 'tab' => 'discussion', 'sort' => 'new']) }}"
               class="rounded-full px-3 py-1" style="{{ $sort === 'new' ? 'background: var(--color-brand-500); color:#04201c' : 'border: 1px solid var(--border)' }}">الأحدث</a>
            <a href="{{ route('volunteer.meetings.show', ['meeting' => $meeting, 'tab' => 'discussion', 'sort' => 'top']) }}"
               class="rounded-full px-3 py-1" style="{{ $sort === 'top' ? 'background: var(--color-brand-500); color:#04201c' : 'border: 1px solid var(--border)' }}">الأعلى تصويتًا</a>
        </div>

        @if ($posts->isEmpty())
            <x-empty message="ابدأ أوّل بوست — النقاش بيبدأ بواحد" />
        @else
            <div class="space-y-3">
    ...
    @push('modals')
        ...
        @if ($canManage)
            <x-modal :id="'questions-'.$meeting->id" title="أسئلة الحضور والـOTP">
                <form method="post" action="{{ route('volunteer.meetings.questions', $meeting) }}" class="space-y-3">
                    @csrf
                    <p class="text-xs" style="color: var(--text-muted)">
                        يضيفها صاحب الاجتماع أو أيّ أبلاين فوقه حتى السقف.
                    </p>
                    ...
                    <fieldset class="rounded-xl p-3" style="border: 1px solid var(--border)">
                        <legend class="text-xs px-1" style="color: var(--text-muted)">سؤال اختيارات جديد</legend>
