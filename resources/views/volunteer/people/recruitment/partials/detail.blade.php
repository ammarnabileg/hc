...
<div class="space-y-4">
    ...
    @if ($line)
        ...
        @if ($canSeeExitReason && $candidate->previous_exit_type)
            <p class="text-xs" style="color: var(--text-muted)">سبب الخروج: {{ $candidate->previous_exit_type }}</p>
        @endif
    @endif

    <dl class="grid grid-cols-2 gap-2 text-sm">
        <dt style="color: var(--text-muted)">درجة شهادة التأهيليّ</dt>
        <dd class="font-semibold">{{ $candidate->qualifying_score ?? '—' }}</dd>
        <dt style="color: var(--text-muted)">مدّة الانتظار</dt>
        <dd>{{ $pipeline->waitingDays($candidate) }} يومًا</dd>
    </dl>
