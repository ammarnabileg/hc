...
@push('head')
    ...
    {{--
        ⭐ **كود هذا البندل داخل `<head>`** — حقلٌ واحد من فورم البندل نفسه.
        ⚠️ يُطبَع **خامًّا** (`{!! !!}`) عن قصد: أمرُ المالك «مسموح أضيف فيهم أي
        حاجة»، والهروب هنا يحوّل الوسم إلى نصٍّ ظاهر فيُبطِل الحقل كلّه. والحارس
        الوحيد أنّ **مالك المنصّة وحده** يملؤه — يُفرَض على الخادم لا هنا.
    --}}
...
@push('scripts')
    {{--
        ⭐ **كود هذا البندل قبل `</body>`** — و`@stack('scripts')` آخر وسمٍ في
        القالب الأمّ قبل الوسم الخاتم، وهذه آخر دفعةٍ في الملفّ فتخرج أخيرًا.
        خامٌّ بلا تعقيم (أمر المالك)، وحارسه الوحيد مالكُ المنصّة على الخادم.
    --}}
