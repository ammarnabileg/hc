{{-- 💖 الخبرة التطوّعيّة: الدور · المنظمة · المدينة · من · إلى · الوصف (الدستور 9) --}}
<div class="card p-3 grid gap-2 sm:grid-cols-2" data-repeat-row draggable="true">
    ...
    <input type="text" name="data[volunteering][{{ $i }}][role]" value="{{ $row['role'] ?? '' }}"
           placeholder="{{ setting('cv.field.volunteer_role_label', 'الدور') }}"
           class="rounded-xl px-3 py-2 text-sm" style="{{ $field }}">

    <input type="text" name="data[volunteering][{{ $i }}][organization]" value="{{ $row['organization'] ?? '' }}"
           placeholder="{{ setting('cv.field.organization_label', 'المنظمة') }}"
           class="rounded-xl px-3 py-2 text-sm" style="{{ $field }}">

    <input type="text" name="data[volunteering][{{ $i }}][city]" value="{{ $row['city'] ?? '' }}"
           placeholder="{{ setting('cv.field.city_label', 'المدينة') }}"
           class="rounded-xl px-3 py-2 text-sm" style="{{ $field }}">

    <div class="flex items-center gap-2">
        <input type="text" name="data[volunteering][{{ $i }}][from]" value="{{ $row['from'] ?? '' }}"
               placeholder="{{ setting('cv.field.from_label', 'من') }}"
               class="w-full rounded-xl px-3 py-2 text-sm" style="{{ $field }}">
        <input type="text" name="data[volunteering][{{ $i }}][to]" value="{{ $row['to'] ?? '' }}"
               placeholder="{{ setting('cv.field.to_label', 'إلى') }}"
               class="w-full rounded-xl px-3 py-2 text-sm" style="{{ $field }}">
    </div>

    <textarea name="data[volunteering][{{ $i }}][description]" rows="2"
    ...
    <textarea name="data[volunteering][{{ $i }}][description_en]" rows="2" dir="ltr" data-lang-en hidden
    ...
    <div class="sm:col-span-2 flex items-center justify-between">
        <label class="flex items-center gap-2 text-xs" style="color: var(--text-muted)">
            <input type="checkbox" name="data[volunteering][{{ $i }}][current]" value="1" @checked(! empty($row['current']))>
