...
<div class="w-full max-w-3xl">
    ...
    <div class="card p-6">
        <x-page-header
            title="{{ \App\Services\Setup\SetupSettings::text('setup.owner_view.title_1', 'حساب مالك المنصّة') }}"
            subtitle="{{ \App\Services\Setup\SetupSettings::text('setup.owner_view.subtitle_1', 'ده حسابك أنت: بيتفعّل على طول وبياخد أعلى صلاحيّة في المنصّة.') }}" />
        ...
        <form method="post" action="{{ route('setup.owner.store') }}" class="space-y-4">
            ...
            <x-form.input name="name" label="{{ \App\Services\Setup\SetupSettings::text('setup.owner_view.label_1', 'الاسم') }}" :value="$draft['name']" required
                          hint="{{ \App\Services\Setup\SetupSettings::text('setup.owner_view.hint_1', 'الاسم اللي هيظهر لك ولزمايلك جوّه المنصّة.') }}" />

            <x-form.input name="email" type="email" label="{{ \App\Services\Setup\SetupSettings::text('setup.owner_view.label_2', 'البريد') }}" dir="ltr" :value="$draft['email']" required
            ...
            <button class="btn rounded-xl px-4 py-2 text-sm font-semibold motion-standard"
                    style="background: var(--color-brand-500); color:#04201c">{{ \App\Services\Setup\SetupSettings::text('setup.owner_view.text_1', 'أنشئ الحساب وكمّل') }}</button>
        </form>
    </div>
</div>
@endsection
