...
class HardcodedTextScanner
{
    ...
    public const EXCLUDED_PATHS = [
        'app/Console' => 'مخرجاتها للطرفيّة (مشغّل النظام) لا لشاشة المستخدم، يعني خارج «النصوص الظاهرة للمستخدم» (2.13-أ). وهذا **الاستثناء الوحيد المخترَع هنا**، وما عداه حدودُ مسحٍ معلَنة لا استثناءات.',
    ];
    ...
    public function exclusionNotes(): array
    {
        $notes = [
            ...
            'التعليقات لا تُحسَب (`{{-- --}}` · `<!-- -->` · `//` · `/* */`)، فالتعليق ليس معروضًا.',
            '`resources/js` و`resources/css` و`public/**` خارج المسح، فالأصول المبنيّة لا تُمسَح.',
            '`database/**` (السيدرات والمايجريشنز) خارج المسح: هي **مصدر القيم الافتراضيّة** نفسه لا حرقًا (BUILD.md §3).',
            '`tests/**` و`lang/**` خارج المسح، فهي ليست شاشةً للمستخدم.',
            ...
            'لا يحكم على **جودة** المفتاح ولا على وجود شاشةٍ تعدّله، بل يحكم على وجود النصّ حرفيًّا فقط.',
            ...
            'مقطعٌ فيه أقلّ من '.self::MIN_LETTERS.' حرفًا عربيًّا لا يُحسَب، فقد يفوته نصٌّ قصير جدًّا.',
            ...
            'رسائل الاستثناءات والسجلّات في `app/` **تُحسَب** وإن كان جمهورها المطوّر، فالفحص لا يقرأ النيّة.',
        ];

        foreach (self::EXCLUDED_PATHS as $path => $why) {
            $notes[] = "`{$path}/` مستثنًى علشان {$why}";
        }
