# 🗂️ `app/Services/Admin` — حالة المجلّد

> وثيقة **القاعدة 2.12** (توثيق التقدّم داخل كلّ مجلّد).
> الأقسام المحصورة بين علامتَي `تلقائيّ:بداية` و`تلقائيّ:نهاية` يعيد كتابتها
> `php artisan docs:status` من الكود نفسه — فلا تُحرّرها بيدك.
> أمّا **المتبقّي** و**الجاري الآن** فبيدك أنت، ولا يمسّهما المولِّد أبدًا:
> حدّثهما **قبل كلّ جلسة عمل وبعدها** كما تنصّ القاعدة.

## 🎯 الغرض/المطلوب
<!-- تلقائيّ:بداية:الغرض -->
- **الوظيفة:** خدمات لوحة الإدارة بمجالاتها (12).
- **البنود الحاكمة (من تعليقات الكود نفسه):** 2.5-د · 2.13-ب · 2.13-هـ · 2.13 · 2.15-د · 2.15 · 2.15-أ-3 · 2.15-أ-7 · 2.15-أ-8 · 2.15-أ-9 · 2.16 · 2.17-ب
- **قواعد سارية على كلّ ما هنا:** الصلاحيّة على كلّ مسار (12.2.1) · المحظور يُخفى لا يُعطَّل (2.15-أ-7) · لا رقم ولا نصّ محروق (2.13) · 🔒 الماليّات لمالك المنصّة وحده (12.7).
<!-- تلقائيّ:نهاية:الغرض -->

## ✅ المُنجَز
<!-- تلقائيّ:بداية:المنجز -->
- **أصناف (6):**
  - `AccountApproval.php` — اعتماد الحسابات (الدستور 2.5-د).
  - `AdminDashboard.php` — لوحة القيادة (الدستور 12.3 · 24.1).
  - `AudienceSegments.php` — شرائح الجمهور (12.13 · 24.1) — شريحة تُبنى بشروط، تُحفَظ باسم، وتُعاد الاستفادة
  - `AuditTrail.php` — سجلّ التدقيق (12.2.1-ز-4 · 2.13-هـ).
  - `RoleEditor.php` — محرّر الأدوار والصلاحيّات (الدستور 12.2 · 12.2.1 · 12.2.3) — أهمّ شاشة في اللوحة.
  - `UserDirectory.php` — قائمة المستخدمين (24.1 · 12.13) — المرجع الواحد لكلّ حسابات المنصّة.
- **مجلّدات فرعيّة (4):** `Content/` · `Ops/` · `System/` · `Volunteer/` — ولكلٍّ منها وثيقتها.
<!-- تلقائيّ:نهاية:المنجز -->

## ⬜ المتبقّي
<!-- بيدك:بداية:المتبقّي -->
### ⬜ نصٌّ محروق باقٍ في هذا المجلّد — بسببه مكتوبًا (دفعة `app/Services` · 2.13)
- ✅ **[مقفولة 2026-08-05] `AuditTrail::ACTIONS` صار `AuditTrail::actions()`.** الشجرة ساكنة فصار القارئ مِلكي: 10 عناوين أفعالٍ عربيّة صارت تُقرَأ من `setting()`، والأكواد وحدها بقيت ثابتًا داخليًّا (`ACTION_KEYS`). حُدِّثت 3 مواضع قراءة (`DashboardController` واحد + قالبان). المفاتيح في `AdminSystemDemoSeeder`. **الدليل:** `php artisan test --filter="AuditTrail|Dashboard"` (43) قبل/بعد بلا فرق؛ وتغيير قيمة `audit.action.role_created` في القاعدة يغيّر عنوان الفلتر فعلًا (مُجرَّب يدويًّا بـ`tinker`).
- ✅ **[مقفولة 2026-08-05] `UserDirectory::COLUMNS`/`STATUSES` صارا `columns()`/`statuses()`.** 10 عناوين أعمدة + 5 حالات حساب صارت من `setting()`، والمفاتيح الداخليّة بقيت `COLUMN_KEYS`/`STATUS_KEYS`. حُدِّثت 7 مواضع قراءة (كنترولر واحد + 5 قوالب). المفاتيح في `AdminCoreDemoSeeder`. **الدليل:** `php artisan test --filter="UserDirectory|Admin.*User"` (31) قبل/بعد بلا فرق.
- ✅ **[مقفولة 2026-09-09] فجوة `settings:coverage --dead`: `admin.segments.refresh_hours` صار له قارئ ومُجدوِل.**
  كان مزروعًا (`AdminCoreDemoSeeder`) بلا قارئ — `AudienceSegments.php` لا تستشير دوريّة تحديث الشريحة
  الديناميكيّة (12.13) في أيّ مسار، ولا Job مجدوَل يقرأها؛ فأثره الوحيد المفقود كان عدّاد «عدد الأعضاء»
  و«آخر تحديث» في قائمة الشرائح يبقى محروقًا على لحظة آخر حفظٍ يدويّ (العضويّة الحيّة نفسها كانت تُعاد
  حسبتها دومًا بصرف النظر — `membersQuery()` لم يتأثّر). أضيفت `AudienceSegments::refreshDue()` على غرار
  `isScheduleDue()`/`isCheckDue()`/`isResetMoment()` (القرار كلّه داخل الخدمة، لا تعبير كرونٍ يتجمّد على
  القيمة القديمة)، ومسحة ساعيّة جديدة في `routes/console.php` (`segments:refresh-dynamic`) تعيد بناء كلّ
  شريحة ديناميكيّة غير مؤرشفة حان موعدها. **الدليل:** `php artisan test --filter=AudienceSegmentsTest`
  (16 اختبارًا، 54 تأكيدًا) — اختباران جديدان يُثبتان أنّ المسحة **مسجَّلة فعلًا** في `routes/console.php`
  (لا وعدًا في التوثيق) وأنّ تشغيلها يحدّث العدّاد المحروق فعلًا، وأنّ الثابتة والمؤرشفة خارج المسحة؛
  وتحوير `isPast()`→`isFuture()` في `refreshDue()` أسقط الاختبار الجديد فورًا (مُثبَتة الفعاليّة).

**أوديت المرحلة 6 — 2026-08-03:**

- ⚠️ **`AdminDashboard`:** «مقارنة بالفترة السابقة على كلّ KPI» (12.3-2) تقع على كارتٍ واحدٍ من أربعة في التشغيل.
- ✅ **[مقفولة 2026-09-11] `AdminDashboard`: كارت المبيعات كان يخرج بقيمته لمن لا يملك الماليّات.** الحراسة كانت على **الرابط وحده** (`urlFor`) والرقم — وهو مجموع الطلبات المدفوعة، أي الإيراد نفسه — يخرج في الوسم لكلّ أدمن؛ فصار الكارت خلف `finance.view` (12.7) كبقيّة الأرقام الماليّة، **يُحذف من المصفوفة ولا يُعطَّل** (2.15-أ-7). والاستعلام نفسه صار داخل الشرط فلا يُحسَب لمن لا يُعرَض له. وبُدِّل فحص `isPlatformOwner()` في `cards()` بمفتاح `finance.view` — **الشاشة نتيجةُ الصلاحيّة لا صلاحيّةٌ بذاتها** (12.2.1-أ)، والمفتاح `is_owner_only` في `database/data/permissions.json` فالأثر واحد والعقد أصدق. **ومعه `series()`:** خطّ «مبيعات» في الرسم كان يحمل الرقم نفسه فصار مشروطًا بالمفتاح كذلك. **الدليل:** 3 اختبارات جديدة في `DashboardScreenTest` (الرقم المنسَّق في التابّين · مفتاح الكارت في المصدر · نقاط الرسم)، وإعادة زرع العيب أسقطت اثنين منها فورًا.
- **مُثبَت شغّالًا:** `AdminDashboard::layoutFor/saveLayout` يحفظ الترتيب والإخفاء **لكلّ دور** في `admin.dashboard.role_layouts` — والمالك رأى ترتيبه المخصّص بينما `support_admin` رأى الافتراضيّ (انتحال حقيقيّ) · `AuditTrail` يسجّل بالمنفِّذ والـIP وقبل/بعد · `AudienceSegments` تُحَلّ على الخادم لحظة الإرسال · `UserDirectory` و`AccountApproval` بصلاحيّاتهما.
<!-- بيدك:نهاية:المتبقّي -->

## 🔄 الجاري الآن
<!-- بيدك:بداية:الجاري -->
- **الحالة:** أوديت فقط — بلا إصلاح.
- **آخر نقطة وصلنا لها:** فُحِصت خدمات اللوحة والتدقيق والشرائح بالتشغيل عبر الشاشات.
- **الخطوة الجاية:** —
<!-- بيدك:نهاية:الجاري -->

## 🔗 التبعيّات والملفّات المهمّة
<!-- تلقائيّ:بداية:التبعيات -->
- **الطبقات الأخرى لنفس المجال:** `app/Http/Controllers/Admin` · `resources/views/admin` · `routes/parts` · `tests/Feature/Admin`
- **الاختبارات:** 2 ملفّ Feature في `tests/Feature/Admin` — شغّلها بـ`php artisan test tests/Feature/Admin`.
- **المرجع الحاكم:** `دستور اساسي.md` · **وكيف نكتب:** `docs/BUILD.md`.
- **المجلّد الأب:** `app/Services/_STATUS.md`.
<!-- تلقائيّ:نهاية:التبعيات -->

## 🕒 آخر تحديث
<!-- تلقائيّ:بداية:التحديث -->
- **آخر توليد لهذه الوثيقة:** 2026-09-09 — `php artisan docs:status`.
- **آخر لمسة للمجلّد:** 2026-09-08 — Claude.
<!-- تلقائيّ:نهاية:التحديث -->
