    ...
    private function record(
        ...
        return DB::transaction(function () use (
            ...
            /*
             ...
             |
             | ================== ولمن **لا** يسري هذا القاع؟ ==================
             | **لقرار الإنسان الموثَّق وحده** — وهما بابان منصوصان:
             |  • **19.4 (تصحيح خطأ تقنيّ):** المعاملة العكسيّة لخصمٍ مكرّر أو
             |    لاسترجاعٍ من البوّابة تُسجَّل كاملةً ولو تركت الرصيد مدينًا،
             |    وإلّا بقي في المحفظة رصيدٌ اعترف الدفتر بأنّه رُدّ.
             |  • **12.9 (إدارة المكافآت):** «**الخصم (ماينص) يقدر ينزل تحت
             |    الصفر** (مسموح عادي)»، و«Toggle **السماح بالنزول تحت الصفر في
             |    الخصم** (**ON — مسموح صراحةً**) 🔒».
             |
             | والفارق بين البابين والخصم الآليّ **مقيسٌ أصلًا** في هذا الملفّ:
             | `documentedHumanDeduction()` — فلا نخترع له معيارًا ثانيًا يتباعد
             | عنه مع الزمن. الخصم بلا توقيع = **آليّ** فيقف عند القاع، والموقَّع
             | بإنسانٍ غير صاحب الرصيد = **قرارٌ موثَّق** فيمضي. وحرّاس الحروب
             | كلّها بلا توقيع (`createdBy = null`) — فالقاع يسري عليها كاملًا.
             */
            $documented = $this->documentedHumanDeduction($user, $createdBy, $source, $isCorrection);

            if ($applied < 0 && ! $documented && $currency->is_spendable && $currency->min_value !== null
            ...
            /*
             | ⭐ وقرارُ الإنسان الموثَّق في العملة القابلة للصرف **لا يُقَصّ عند
             | القاع** كذلك — وإلّا لانقلب الاستثناء قصًّا صامتًا:
             |  • **19.4:** شحن 100 · صرف 90 · ثمّ استرجاعٌ من البوّابة ⟵ العكسيّة
             |    −100 كاملة ورصيدٌ **−10** يقول الحقيقة؛ أمّا القصّ عند صفر
             |    فيسجّل −100 ويطبّق −10: دفترٌ يفارق الرصيد وهو ما نغلقه لا نفتحه.
             |  • **12.9:** خصمُ الأدمن اليدويّ 250 من رصيدٍ صفر ⟵ **−250** كما نصّ
             |    «الخصم يقدر ينزل تحت الصفر»، لا صفرًا وسطرًا يدّعي 250.
             | و`rep` غير قابلة للصرف فتبقى مقصوصةً بسقفها المنصوص أبدًا (13.4-ن).
             */
            if ($currency->min_value !== null && ! ($documented && $currency->is_spendable)) {
