...
class QuestionBank
{
    ...
    /**
     * ⭐ **الأسئلة كما سيراها المتدرّب فعلًا في امتحانٍ بعينه** — مصدر الحقيقة
     * الواحد لبناء الامتحان النهائيّ.
     *
     * لماذا هنا لا في شاشة الامتحان وحدها؟ لأنّ 12.4-هـ تطلب **«معاينة الامتحان
     * النهائيّ كما سيُبنى»**، و«كما سيُبنى» تعني حرفيًّا: **نفس الاختيار الذي
     * ينفّذه المبنيّ**. فلو نسخت المعاينةُ الاستعلامَ لنفسها لافترقت عنه بأوّل
     * تعديل، وصارت تَعِد الأدمن بامتحانٍ غير الذي يمتحنه الناس — وهو أسوأ من
     * غياب المعاينة. فالقارئان (شاشة الامتحان 24.5 · 4.2، ومعاينة الأدمن) يقرآن
     * من هنا، فلا يفترقان أبدًا.
     *
     * @return Collection<int,ExamQuestion>
     */
    public function builtQuestions(Exam $exam): Collection
    {
        return ExamQuestion::query()
            ->where('exam_id', $exam->id)
            ->orderBy('sort_order')
            ->orderBy('id')
            ->take($this->builtLimit($exam))
            ->get();
    }

    /**
     * ⭐ عدد أسئلة الامتحان: قيمة التدريب المحدَّدة في تاب «التقييم» أوّلًا
     * (`exams.questions_count`)، وإلّا الحدّ العامّ الحاكم (4.2 · 2.13).
     */
    public function builtLimit(Exam $exam): int
    {
        return (int) $exam->questions_count ?: (int) setting('exams.questions.max', 20);
    }
