...
class AccountContainmentGate
{
     ...
     *  1) **الخروج** فحقّ المستخدم يقفل جلسته مهما كانت حالته، وإلّا حبسناه فيها.
     *  2) **إنهاء الانتحال**: لو أدمن كان بيتصفّح كحساب محتوًى، لازم يقدر يرجع
     *     لحسابه — الجدار على المحظور لا على مَن يفحص حالته.
     *  3) مسارات الصحّة والويب هوك — خارج جلسة المستخدم أصلًا.
     *
     * والجدار يُرسَم **مكان الصفحة المطلوبة** لا بتحويلة، فلا حلقةَ تحويلٍ أصلًا.
     */
    ...
    public function exemptPaths(): array
    {
        ...
        return ['logout', 'impersonate/stop', 'up', 'webhooks/*'];
    }
