...
- **4.2 — سقف الأسئلة يقرأ الإعداد العامّ لا إعداد الامتحان:** `ExamController::questions()` يقرأ `setting('exams.questions.max')` ويتجاهل `exams.questions_count` الذي يكتبه الأدمن.
- **6 — لا عدّاد حيّ:** `DeadlineService` يُرجع `seconds_left` ولا قارئ له في صفحة التدريب؛ النصّ ثابت والشبح بموضعٍ محسوبٍ لحظة التحميل فقط. والدستور: «موعد نهائيّ مع **عدّاد تنازليّ تفاعليّ**».
...
## 🔄 الجاري الآن
<!-- بيدك:بداية:الجاري -->
- **الحالة:** أُغلقت فجوتا الإتاحة (الملاحظات + بلوك الامتحان) — بإثبات HTTP وبطفرتين تُسقِطان الاختبار.
- **آخر نقطة وصلنا لها:** 2026-08-03 — طلبات حقيقيّة على خادم بقاعدةٍ مبذورة، ومتصفّح على 375px.
- **الخطوة الجاية:** الكونفيتي (4.1) · عدّاد الديدلاين الحيّ (6) · دينامكيّة الدولة (5).
<!-- بيدك:نهاية:الجاري -->
...
- **الاختبارات:** 18 ملفّ Feature في `tests/Feature/Learning` — شغّلها بـ`php artisan test tests/Feature/Learning`.
