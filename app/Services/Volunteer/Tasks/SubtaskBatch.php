...
class SubtaskBatch
{
    ...
    public function validate(Task $parent, array $rows): void
    {
        ...
        foreach ($rows as $index => $row) {
            if (blank($row['deadline_at'] ?? null)) {
                throw ValidationException::withMessages([
                    "subtasks.{$index}.deadline_at" => setting('workflow.subtask_batch.validate_2', 'كلّ صب-تاسك لازم له ديدلاين داخليّ.'),
                ]);
            }
    ...
    /**
     * ⭐ هل هذا البند مؤهّل لفتح دعوة مساهم عليه؟ (23-2.3-٥ · 23-4-الأساس)
     *
     * النصّ حرفيًّا: «بعد الاعتماد: الصب-تاسك بند حقيقي… أو تُفتَح عليه دعوة
     * مساهم… دعوة المساهم لا تُفتَح إلا على صب-تاسك معتمد» (23-2.3-٥)، و«الدعوة
     * تُفتَح على صب-تاسك معتمد (مرّ بدورة القسم 2.3) — أي بوزشن، حتى بلا فريق»
     * (23-4-الأساس).
     *
     * فالقيد على **الصب-تاسك** تحديدًا (له `parent_task_id`) — هو مَن يمرّ
     * بدورة الإنشاء دفعةً ⟵ حفظ للمراجعة ⟵ اعتماد محرّك التصعيد (23-2.3)،
     * ويحمل `batch_status` (مسودّة/بانتظار مراجعة الأبلاين/معتمدة). أمّا
     * **المهمّة الجذر** (بلا أب) فلم تدخل هذه الدورة أصلًا — عمودها
     * `batch_status` يبقى `null` طول عمرها — فلا معنى لقياسها على شرطٍ لم
     * تخضع له، وتبقى دعوة المساهم عليها متاحة كسابق عهدها.
     *
     * هذه هي نقطة تنفيذ شرط المصفوفة `state:subtask_approved` (`config/access.php`
     * يَسِمه `guard => 'domain'` عمدًا: حالةٌ **لا يقرأها عمود حالة عامّ** —
     * يحرسها المجال صاحب الشاشة، وهذا المجال).
     */
    public function isApprovedForInvite(Task $task): bool
    {
        return ! $task->parent_task_id || $task->batch_status === 'approved';
    }
    ...
    private function chargeBreakdownDelay(Task $parent, User $author, bool $alreadyBrokenDown = false): void
    {
        ...
        RepOnce::record(
            ...
            fn () => app(LedgerBridge::class)->record(
                ...
                $value,
                'task',
                strtr(setting('workflow.subtask_batch.charge_breakdown_delay_1', 'تأخّر التفكيك :p1 يومًا على: :p2'), [':p1' => (string) ($days), ':p2' => (string) ($parent->title)]),
                $parent,
                $parent->entity_id,
            ),
        );
