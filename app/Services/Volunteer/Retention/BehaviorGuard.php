...
class BehaviorGuard
{
    ...
    public function assertScope(User $granter, User $target): void
    {
        ...
        throw new RuntimeException(
            setting('volunteer_offboarding.behavior_guard.assert_scope_1', 'العضو ده مش في داونلاينك داخل عضويّتك النشطة، ومعاملة السلوك لداونلاينك في كيانك وحدهم.')
        );
    }
    ...
    public function assertNotDuplicated(User $target, BehaviorViolation $violation, string $incidentRef): void
    {
        ...
        if ($exists) {
            throw new RuntimeException(
                setting('volunteer_offboarding.behavior_guard.assert_not_duplicated_1', 'الواقعة دي متسجّلة على العضو بنفس المخالفة قبل كده، ومعاملة واحدة لكلّ واقعة.')
            );
        }
    }
}
