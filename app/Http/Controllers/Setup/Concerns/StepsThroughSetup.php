...
trait StepsThroughSetup
{
    ...
    protected function guardStep(SetupState $state, string $step): ?RedirectResponse
    {
        ...
        return redirect()->route($this->routeFor($next))->withErrors([
            'setup' => strtr((string) SetupSettings::text('setup.steps.guard_step_must', 'لازم تخلّص «:a1» الأوّل عشان نكمل بأمان، علشان كده رجّعناك لمكانها.'), [':a1' => (string) ($this->stepLabel($next))]),
        ]);
    }
