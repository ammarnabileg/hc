# 🗂️ `app/Http/Controllers/Api` — حالة المجلّد
...
## 🎯 الغرض/المطلوب
<!-- تلقائيّ:بداية:الغرض -->
- **الوظيفة:** «Api» ضمن: المتحكّمات — تتحقّق وتنسّق وترجع View، والقرار في الخدمة لا هنا.
...
- **مجلّدات فرعيّة (1):** `V1/` — ولكلٍّ منها وثيقتها.
<!-- تلقائيّ:نهاية:المنجز -->

## ⬜ المتبقّي
<!-- بيدك:بداية:المتبقّي -->
- ما فيش فجوة معروفة في هذا المجلّد وقت إنشاء الوثيقة.
- **هذا المجلّد يخدم مستهلكين خارجيّين لا متصفّح المستخدم** (12.15-أ): المصادقة بمفتاح API عبر ميدل-وير `api.key` (`AuthenticateApiKey`) لا بجلسة/`auth`. ثلاث نقاط نهاية فقط الآن (`ping` · `courses` · `certificates/{code}/verify`) — كتالوجها الكامل والموثَّق في `App\Services\Developers\ApiEndpointCatalog`.
