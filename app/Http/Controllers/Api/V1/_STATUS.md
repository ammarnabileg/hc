# 🗂️ `app/Http/Controllers/Api/V1` — حالة المجلّد
...
## 🎯 الغرض/المطلوب
<!-- تلقائيّ:بداية:الغرض -->
- **الوظيفة:** `GET /api/v1/certificates/{code}/verify` — Scope `read:certificates` (12.15-أ).
- **البنود الحاكمة (من تعليقات الكود نفسه):** 2.11 · 12.15-أ
...
- **أصناف (3):**
  - `CertificateVerificationController.php` — `GET /api/v1/certificates/{code}/verify` — Scope `read:certificates` (12.15-أ).
  - `CoursesController.php` — `GET /api/v1/courses` — Scope `read:courses` (12.15-أ).
  - `PingController.php` — `GET /api/v1/ping` — بلا Scope، يتطلّب مفتاحًا صالحًا فقط (12.15-أ).
...
- **الطبقات الأخرى لنفس المجال:** `app/Http/Controllers/Api` · `routes/parts`
...
- **المجلّد الأب:** `app/Http/Controllers/Api/_STATUS.md`.
