    ...
    public function settings(): void
    {
        $rows = [
            ...
            */
            ...
            ['store.admin.bundles.code_when_label', 'وسم خانة «متى يُحقَن؟»', 'string', 'متى يُحقَن؟'],
            ['store.admin.bundles.code_slot_labels', 'تسميات موضعَي حقن الكود', 'json', json_encode([
                'head' => 'كود داخل <head> لهذا البندل',
                'body_end' => 'كود قبل </body> لهذا البندل',
            ], JSON_UNESCAPED_UNICODE)],
            ...
            /*
            |----------------------------------------------------------------
            | ⭐ [كود مخصّص] — العامّ لكلّ صفحات البندلات (أمر المالك)
            |----------------------------------------------------------------
            | 🔒 يحرّره **مالك المنصّة وحده**، ويُحقَن **خامًّا بلا تعقيم**، ولا
            | يُحقَن إلّا بعد اجتياز خانة «متى يُحقَن؟» — والافتراضيّ **الأضيق**.
            */
            ['store.bundle.head_code', 'كود مخصّص في <head> لكلّ البندلات 🔒', 'text', ''],
            ['store.bundle.head_code_when', 'متى يُحقَن كود الهيد العامّ؟', 'string', 'ads'],
            ['store.bundle.body_end_code', 'كود مخصّص قبل </body> لكلّ البندلات 🔒', 'text', ''],
            ['store.bundle.body_end_code_when', 'متى يُحقَن كود نهاية البودي العامّ؟', 'string', 'ads'],
            ['store.bundle.code_when_labels', 'خيارات خانة «متى يُحقَن؟»', 'json', json_encode([
                'always' => 'دائمًا — لكلّ زائر',
                'analytics' => 'بعد موافقة القياس',
                'ads' => 'بعد موافقة الإعلان',
            ], JSON_UNESCAPED_UNICODE)],
            ['store.bundle.code_consent_note', 'ملاحظة الموافقة تحت حقول الكود', 'text', 'لو الكود بكسل تتبّع أو إعلان، سيبه على خيار الموافقة — حقنُه دائمًا يخالف وعد بانر الخصوصيّة للمستخدم، وبيخلّي الرفض بلا أثر فعليّ.'],
