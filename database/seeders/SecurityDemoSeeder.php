...
class SecurityDemoSeeder extends Seeder
{
    ...
    public function settings(): void
    {
        $rows = [
            ...
            ['auth.otp.length', 'security', 'عدد أرقام رمز التحقّق', 'number', '4'],
            ...
            ['auth.otp.code_label', 'security', 'عنوان حقل الرمز', 'string', 'كود التحقّق'],
            ['auth.otp.back_label', 'security', 'رابط تعديل البريد', 'string', 'ارجع عدّله'],
            ['auth.otp.wrong_email_hint', 'security', 'سؤال تعديل البريد', 'string', 'البريد غلط؟'],
            ['auth.otp.sent_text', 'security', 'رسالة بعد الإرسال', 'text', 'بعتنا الرمز على بريدك. بصّ في «غير الهامّ» كمان.'],
            ['auth.otp.wait_text', 'security', 'رسالة انتظار إعادة الإرسال', 'text', 'استنّى {seconds} ثانية قبل ما تطلب تاني.'],
            ['auth.otp.error_missing', 'security', 'خطأ: مافيش رمز', 'text', 'مابعتناش رمز لسّه. اضغط «إرسال» الأوّل.'],
            ['auth.otp.error_wrong', 'security', 'خطأ: رمز غلط', 'text', 'الرمز مش مظبوط. راجع بريدك وجرّب تاني.'],
            ...
            // نفسه لا خيار فيه — لكنّ الحدّ لكلّ IP في نافذة يمنع الجرد بالجملة (سائلٌ آليّ
            // بمليون بريد يخرج بقائمة أعضاء المنصّة). رُدِمت هذه الثلاثة سابقًا بشبكة الأمان
            // التلقائيّة (SettingGapSeeder) بلافتاتٍ مشتقّة آليًّا — نُقلت هنا بلافتاتٍ عربيّة.
            ['auth.email_probe.max_per_window', 'security', 'أقصى محاولات جرد بريد لكلّ IP في النافذة', 'number', '30'],
            ['auth.email_probe.window_seconds', 'security', 'مدّة نافذة حدّ جرد البريد (ثوانٍ)', 'number', '60'],
            ['auth.email_probe.throttled', 'security', 'رسالة تجاوز حدّ جرد البريد', 'string', 'استنّى شويّة وجرّب تاني.'],
            ...
            ['auth.password_reset.sent_hint', 'security', 'شرح شاشة الإرسال', 'text', 'لو {email} مسجّل عندنا، هتلاقي رابط ورمز. صالحين {minutes} دقيقة.'],
            ['auth.password_reset.code_label', 'security', 'عنوان حقل الرمز', 'string', 'أو اكتب الرمز اللي وصلك'],
            ...
            ['account.delete.data_notes', 'account', 'ما يحدث للبيانات', 'json', json_encode([
                ...
                'شهاداتك الصادرة هتفضل قابلة للتحقّق برقمها، علشان دي حقّ الجهة اللي استلمتها.',
            ...
            /*
             | ---------------- سلّة المحذوفات الموحّدة (12.2.2 سطر 2188-2191 · 5169)
             | نفس مهلة الـ30 يومًا المنصوصة حرفيًّا («Soft-delete = 30 يومًا ثمّ
             | حذف نهائيّ» — سطر 5169) — رقمٌ واحد يقرؤه `TrashRecovery` لكلّ
             | الموارد السبعة، لا نسخة محروقة لكلّ موديل.
             */
            ['admin.trash.retention_days', 'security', 'مهلة الاحتفاظ بالمحذوف قبل الحذف النهائيّ (يوم)', 'number', '30'],
